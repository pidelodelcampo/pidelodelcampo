...
function getVipConfirmado(item: Producto) {
  ...
  if (item.payment_status) return item.payment_status === 'approved'
  return !!item.vip_activo
}

function getEstadoVipLabel(item: Producto) {
  const pago = (item.payment_status || '').toLowerCase()
  ...
  if (item.plan === 'vip') {
    ...
    if (!pago && item.vip_activo) return 'VIP ACTIVO'
...
export default function PerfilPage() {
  ...
  const vipPendientes = useMemo(
    () =>
      productos.filter(
        (p) => p.plan === 'vip' && !getVipConfirmado(p)
      ).length,
    [productos]
  )
  ...
  const perfilChecklist = useMemo(
    () => [
      { label: 'Nombre del vendedor', ok: perfil.nombre_vendedor.trim().length >= 3 },
      ...
      { label: 'Avatar', ok: !!perfil.avatar_url },
    ],
    [perfil]
  )
    ...
    const load = async () => {
      ...
      if (ids.length > 0) {
        ...
        ;(imagesData || []).forEach((img: any) => {
          if (!imagesMap[img.product_id]) imagesMap[img.product_id] = img.image_url
        })
        ...
        ;(pagosData || []).forEach((pago: any) => {
          if (!paymentMap[pago.product_id]) {
            paymentMap[pago.product_id] = pago.payment_status || ''
          }
        })
      }
      ...
      if (perfilData) {
        setPerfil({
          nombre_vendedor: perfilData.nombre_vendedor || '',
          ...
          plan_actual: perfilData.plan_actual || 'free',
        })
      } else {
        setPerfil((prev) => ({
          ...prev,
          nombre_vendedor: authData.user.user_metadata?.nombre_negocio || '',
      ...
      setProductos(
        products.map((p: any) => ({
          ...
          payment_status: paymentMap[p.id] || null,
        }))
      )
  ...
  const guardarPerfil = async () => {
    ...
    const payload = {
      user_id: user.id,
      nombre_vendedor: perfil.nombre_vendedor || 'Vendedor',
      ...
      <div style={s.wrap}>
        <section style={s.hero} className="glass-pulse">
          ...
          <div style={s.heroContent}>
            ...
            <p style={s.heroText}>
              Completa tus datos, fortalece tu confianza y revisa el estado real de tus publicaciones y pagos VIP.
            </p>
        ...
        <div className="perfil-layout">
          <section style={s.card}>
            <div style={s.profileHead}>
              <div style={s.avatarWrap}>
                {perfil.avatar_url ? (
                  <img src={perfil.avatar_url} alt="Avatar" style={s.avatar} />
                ) : (
                  <div style={s.avatar}>
                    {initials(perfil.nombre_vendedor, user?.email || '')}
                  </div>
                )}
              ...
              <div>
                ...
                <p style={s.text}>
                  Entre más completo esté tu perfil, más confianza genera y mejor se ve tu presencia en la plataforma.
                </p>
            ...
            <div style={s.form}>
              <div style={s.row2} className="perfil-row-2">
                <div style={s.field}>
                  ...
                  <input
                    style={s.input}
                    value={perfil.nombre_vendedor}
                    onChange={(e) =>
                      setPerfil((p) => ({
                        ...p,
                        nombre_vendedor: capitalizeWords(e.target.value),
                      }))
                    }
          ...
          <aside className="sticky-summary" style={s.asideCard}>
            ...
            <div className="stats-grid-enhanced" style={{ marginTop: 0 }}>
              ...
              <div style={s.stat}>
                <div style={s.statNum}>{productos.filter((p) => p.estado === 'published').length}</div>
                <div style={s.statLabel}>Publicadas</div>
              </div>
            </div>

            <div style={s.infoBox}>
              👑 Una publicación solo debe verse como VIP cuando el pago esté realmente confirmado. Si el pago sigue pendiente, aquí se muestra como pendiente y no como VIP activo.
            </div>
            ...
            >
              ...
              <div style={{ fontSize: '14px', color: '#374151', lineHeight: 1.7 }}>
                <div><strong>Negocio:</strong> {perfil.nombre_vendedor || 'Sin nombre aún'}</div>
                <div><strong>Ciudad:</strong> {perfil.municipio || 'Sin municipio'} · {perfil.departamento}</div>
                <div><strong>Teléfono:</strong> {perfil.telefono || 'Sin teléfono'}</div>
                <div><strong>Email:</strong> {perfil.email || user?.email || 'Sin correo'}</div>
              </div>
            </div>
        ...
        <section style={{ ...s.card, marginTop: '14px' }}>
          ...
          <p style={s.text}>
            Aquí ves tus productos con su estado real, incluyendo si el VIP ya está aprobado o sigue pendiente.
          </p>
                ...
                return (
                  <article key={item.id} style={s.productCard}>
                    ...
                    <div>
                      ...
                      >
                        Publicado el {formatShortDate(item.created_at)}
                      </div>

                      <div style={{ ...s.productMeta, marginTop: '10px' }}>
                      ...
                      {item.plan === 'vip' && !getVipConfirmado(item) ? (
                        <div style={s.infoBox}>
                          Esta publicación aún no aparece como VIP activa. Se mantiene pendiente hasta la confirmación del pago.
                        </div>
                      ) : null}
